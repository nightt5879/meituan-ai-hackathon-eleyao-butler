...
export const techRouteData = {
  funcs: [
    ...
    },
    {
      ...
      oneLiner: "发起人建任务、成员各填偏好；每项条件由用户自己标记为「必须满足」或「希望满足」，管家再拆成硬约束与软偏好，显式指出冲突，用最低满意度而非平均分选出最稳的一家，再生成能直接发群里的话。",
      stages: [
        {
          key: "in",
          title: "收集偏好",
        ...
        },
        {
          ...
          sub: "PROCESS",
          nodes: [
            { t: "自定义权重", desc: "必须满足 vs 希望满足", detail: "时间、忌口/过敏、品类、预算、距离、环境偏好都由用户自己选择强度；必须满足进入硬性过滤，希望满足进入推荐排序。" },
      ...
      fields: [
        ...
        { name: "约束分流", note: "由用户选择决定优先级", items: ["必须满足 → 硬约束 / 不可违反", "希望满足 → 软偏好 / 尽量满足"] }
      ],
      features: [
  ...
  selfcheck: {
    ...
    checks: [
      ...
      { k: "用户底线", look: "是否违反用户标记为必须满足的忌口、过敏、预算、时间等条件", act: "直接淘汰或重新生成候选" },
