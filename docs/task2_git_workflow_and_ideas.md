...
![[Pasted image 20260423192952.png]]
...
ps：这个review操作，以后大家都可以看，看看某个分支具体实现了什么东西，更新了什么
...
1. **永远不要直接在 `main` 分支 commit**。先切到自己的分支再改（规则锁了，想要这样搞也搞不了）
2. **Commit message 要写人话**。写"修改"、"aa" 这种 = 不合格，要写清楚。
...
卡住超过 20 分钟 → 群里截图+说明情况 @我

# 关于线下/线上统一讨论
1. 首先完成这个任务，大家都提交一下文字记录的看法
2. 然后大家都合入了main分支，各自都看过大家的想法
3. 此时在同一进行讨论。可以选择周五晚（如果任务都做完并且适合）或 周六线下顺便讨论
